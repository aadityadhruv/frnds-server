...
const sendRequest = async (id, friendId) => {
  var user = await User.findByIdAndUpdate(id, {
    $push: { requestsSent: friendId },
  });
  var friend = await User.findByIdAndUpdate(friendId, {
...
const friendsInClass = async (id, className) => {
  ...
  for(var i =0;i<user.friends.length;i++){
    var friend = await User.findById(user.friends[i]);
    if(friend.classes.includes(className)){
      val.push(user.friends[i]);
    }
  }
  return val
};
